...
****** JavaScript
#+BEGIN_SRC js
const ws = new WebSocket(`ws://localhost:8080/logger/${id}`)
...
****** 获取JSON Schema
=GET /admin/logger/<id>/json-schema=
...
curl \
  --header "Authorization: Bearer $ADMIN_PASSWORD" \
  "http://localhost:8080/admin/logger/$id/json-schema"
#+END_SRC

******** fetch
#+BEGIN_SRC js
await fetch(`http://localhost:8080/admin/logger/${id}/json-schema`, {
  headers: {
    'Authorization': `Bearer ${adminPassword}`
  }
}).then(res => res.json())
...
****** 设置JSON Schema
=PUT /admin/logger/<id>/json-schema=
...
curl \
  ...
  --data "$JSON_SCHEMA" \
  "http://localhost:8080/admin/logger/$id/jsonschema"
#+END_SRC
...
****** 移除JSON Schema
=DELETE /admin/logger/<id>/json-schema=
...
****** 获取清除策略
=GET /admin/logger/<id>/purge-policies=
...
curl \
  --header "Authorization: Bearer $ADMIN_PASSWORD" \
  "http://localhost:8080/admin/logger/$id/purge-policies"
#+END_SRC

******** fetch
#+BEGIN_SRC js
await fetch(`http://localhost:8080/admin/logger/${id}/purge-policies`, {
  headers: {
    'Authorization': `Bearer ${adminPassword}`
  }
}).then(res => res.json())
...
****** 设置清除策略
=PUT /admin/logger/<id>/purge-policies/time-to-live=
=PUT /admin/logger/<id>/purge-policies/limit=
...
curl \
  ...
  --header "Authorization: Bearer $ADMIN_PASSWORD" \
  --data "$LIMIT" \
  "http://localhost:8080/admin/logger/$id/limit"
#+END_SRC

******** fetch
#+BEGIN_SRC js
await fetch(`http://localhost:8080/admin/logger/${id}/limit`, {
  method: 'PUT'
, headers: {
    'Authorization': `Bearer ${adminPassword}`
  }
...
****** 移除清除策略
=DELETE /admin/logger/<id>/purge-policies/time-to-live=
=DELETE /admin/logger/<id>/purge-policies/limit=
...
****** 手动触发清除策略
=POST /admin/logger/<id>/purge-policies=
...
****** 获取特定频道的token策略
=GET /admin/logger/<id>/token-policies=
...
curl \
  --header "Authorization: Bearer $ADMIN_PASSWORD" \
  "http://localhost:8080/admin/logger/$id/token-policies"
#+END_SRC

******** fetch
#+BEGIN_SRC js
await fethc(`http://localhost:8080/admin/logger/${id}/token-policies`, {
  headers: {
    'Authorization': `Bearer ${adminPassword}`
  }
}).then(res => res.json())
...
****** 设置token策略
=PUT /admin/logger/<id>/token-policies/write-token-required=
=PUT /admin/logger/<id>/token-policies/read-token-required=
=PUT /admin/logger/<id>/token-policies/delete-token-required=
...
curl \
  ...
  "http://localhost:8080/admin/logger/$id/token-policies/write-token-required"
#+END_SRC

******** fetch
#+BEGIN_SRC js
await fetch(`http://localhost:8080/admin/logger/${id}/token-policies/write-token-required`, {
  method: 'PUT'
, headers: {
    'Authorization': `Bearer ${adminPassword}`
  , 'Content-Type': 'application/json'
...
****** 移除token策略
=DELETE /admin/logger/<id>/token-policies/write-token-required=
=DELETE /admin/logger/<id>/token-policies/read-token-required=
=DELETE /admin/logger/<id>/token-policies/delete-token-required=
...
curl \
  --header "Authorization: Bearer $ADMIN_PASSWORD" \
  "http://localhost:8080/admin/logger/$id/tokens"
#+END_SRC

******** fetch
#+BEGIN_SRC js
await fetch(`http://localhost:8080/admin/logger/${id}/tokens`, {
  headers: {
    'Authorization': `Bearer ${adminPassword}`
  }
}).then(res => res.json())
...
=PUT /admin/logger/<id>/tokens/<token>/write=
...
curl \
  --request PUT \
  --header "Authorization: Bearer $ADMIN_PASSWORD" \
  "http://localhost:8080/admin/logger/$id/tokens/$token/log"
#+END_SRC

******** fetch
#+BEGIN_SRC js
await fetch(`http://localhost:8080/admin/logger/${id}/tokens/${token}/log`, {
  method: 'PUT'
, headers: {
    'Authorization': `Bearer ${adminPassword}`
  }
...
=DELETE /admin/logger/<id>/tokens/<token>/write=
...
curl \
  --request DELETE \
  --header "Authorization: Bearer $ADMIN_PASSWORD" \
  "http://localhost:8080/admin/logger/$id/tokens/$token/write"
#+END_SRC

******** fetch
#+BEGIN_SRC js
await fetch(`http://localhost:8080/admin/logger/${id}/tokens/${token}/write`, {
  method: 'DELETE'
, headers: {
    'Authorization': `Bearer ${adminPassword}`
  }
...
=PUT /admin/logger/<id>/tokens/<token>/read=
...
curl \
  --request PUT \
  --header "Authorization: Bearer $ADMIN_PASSWORD" \
  "http://localhost:8080/admin/logger/$id/tokens/$token/read"
#+END_SRC

******** fetch
#+BEGIN_SRC js
await fetch(`http://localhost:8080/admin/logger/${id}/tokens/${token}/read`, {
  method: 'PUT'
, headers: {
    'Authorization': `Bearer ${adminPassword}`
  }
...
=DELETE /admin/logger/<id>/tokens/<token>/read=
...
=PUT /admin/logger/<id>/tokens/<token>/delete=
...
curl \
  --request PUT \
  --header "Authorization: Bearer $ADMIN_PASSWORD" \
  "http://localhost:8080/admin/logger/$id/tokens/$token/delete"
#+END_SRC

******** fetch
#+BEGIN_SRC js
await fetch(`http://localhost:8080/admin/logger/${id}/tokens/${token}/delete`, {
  method: 'PUT'
, headers: {
    'Authorization': `Bearer ${adminPassword}`
  }
...
=DELETE /admin/logger/<id>/tokens/<token>/delete=
