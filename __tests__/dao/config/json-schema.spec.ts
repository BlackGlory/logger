...
function insert(db: Database, { id, schema }:{ id: string; schema: string }) {
  db.prepare('INSERT INTO logger_json_schema (logger_id, json_schema) VALUES ($id, $schema);').run({ id, schema })
}

function select(db: Database, id: string) {
  return db.prepare('SELECT * FROM logger_json_schema WHERE logger_id = $id;').get({ id })
}
