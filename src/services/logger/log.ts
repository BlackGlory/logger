...
export const routes: FastifyPluginAsync<{
  ...
  , async (req, reply) => {
      ...
      if (TOKEN_BASED_ACCESS_CONTROL()) {
        if (await DAO.hasLogTokens(id)) {
          if (token) {
            if (!await DAO.matchLogToken({ token, id })) return reply.status(401).send()
          } else {
            return reply.status(401).send()
          }
        } else {
          if (DISABLE_NO_TOKENS()) {
            if (!await DAO.hasFollowTokens(id)) return reply.status(403).send()
          }
        }
      }
      ...
      await Logger.log(req.params.id, req.body)
